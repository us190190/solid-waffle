...
async def lifespan(application: FastAPI):
    ...
    try:
        ...
        print("Travel Planner shutdown - cleanup complete")


app = FastAPI(
    title="Travel Planner",
    description="Hierarchical Teams: TripManager (Command delegation) -> FlightSubgraph (Search->Filter->Rank) -> HotelSubgraph -> ItinerarySubgraph -> Summarize (AsyncSqliteSaver + InMemoryStore, thread_id refine).",
    version="0.1.0",
    lifespan=lifespan,
)
...
async def root_health():
    return {"status": "ok", "gemini_configured": has_gemini_key(), "model": settings.gemini_model,
            "checkpointer": "AsyncSqliteSaver", "store": "InMemoryStore"}
